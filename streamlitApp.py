...
# Path of SVM model and text and it's name
pkl = 'model/svm_model.pkl'
csv = 'test/Xy_test.csv'
model_name = 'SVM'
...
df, features_df, labels = read_test_set(csv)
...
def title(model_name):
    st.markdown(f"<h1 style='text-align: left; color: red;'>Face Recgonition using {model_name}</h1>", unsafe_allow_html=True)


# Choose from SVM, Random Forest or DecisionTree
imported_data = st.selectbox('Model', ('SVM', 'Random Forest', 'DecisionTree'))
if(imported_data == 'SVM'):
    pkl = 'model/svm_model.pkl'
    csv = 'test/Xy_test.csv'
...
elif(imported_data == 'Random Forest'):
    pkl = 'model/rf_model.pkl'
    csv = 'test/rf_test.csv'
    ...
    pkl = 'model/clf_model.pkl'
    csv = 'test/clf_test.csv'
